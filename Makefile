...
node-check:
	...
	@if [ "$(NODE_VERSION)" -lt "$(MIN_NODE_VERSION)" -o "$(NPM_MISSING)" = "1" ]; then \
		echo "Gitea requires Node.js $(NODE_VERSION) $(MIN_NODE_VER_FMT) or greater and npm to build. You can get it at https://nodejs.org/en/download/"; \
		exit 1; \
	fi
...
.PHONY: docker
docker:
	docker build --disable-content-trust=true -t $(DOCKER_REF) .
